    ...
    classificationBadges.forEach(badge => {
      ...
      if (hasMatchingClassification) {
        ...
        degreeRowBottoms.forEach(bottomRow => {
          const bottomBadge = bottomRow.querySelector('.degree-classification');
          if (bottomBadge && bottomBadge.classList.contains(filterType)) {
...
function clearFilter() {
  ...
  const letterGroups = document.querySelectorAll('.lettergroup');
    ...
    degreeRows.forEach(row => {
      ...
      // Show all degree-row-bottom elements when clearing filters
      const degreeRowBottoms = row.querySelectorAll('.degree-row-bottom');
      degreeRowBottoms.forEach(bottomRow => {
        bottomRow.style.display = 'block';
      });
...
function showFilterFeedback(filterType) {
  ...
  indicator.innerHTML = `
        ...
        <button type="button" class="clear-filter-btn" onclick="clearAllFilters()">✕ Show All</button>
    `;
